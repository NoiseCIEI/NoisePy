...
int main(int na, char *arg[])
{
    if(na!=14)
    {
        cout<<"Usage: Slowness2IsoAniMap station.lst min max N_bin data_pre out_pre dx x0 nx y0 ny pflag cridist"<<endl;
        return 0;
    }
    FILE *ff,*fin,*fout,*file1,*file_iso,*file_ani;
    int i,j,k;
    int npts_x,npts_y;
    char buff1[300],sta1[10],namem[100], namemu[100], name_iso[100],name_ani[100],name_ani_n[100],pflag[5];
    ...
    max=atof(arg[3]);
    N_bin=atoi(arg[4]);
    sprintf(namem,"%sall.measurement.dat",arg[6]);
    sprintf(namemu,"%sall.measurement.used.dat",arg[6]);
    sprintf(name_iso,"%sisomap",arg[6]);
    sprintf(name_ani,"%sanimap",arg[6]);
    ...
    radius=6371.1391285;
    pi=4.0*atan(1.0);
    double dx,dy,x0,y0,x1,y1,temp,lat_temp,temp2,trash1,trash2;
    ...
    double cvlon,cvlat,cridist,tdist;

    FILE *fall,*fall1;
    fall = fopen(namem,"w");
    fall1 = fopen(namemu,"w");
    ...
    for(i=0; i<npts_x; i++)
    {
        for(j=0; j<npts_y; j++)
        {
            ...
            for(k=0; k<n[i][j]; k++)
            {
                ...
                weight[k]=1/weight[k]; // !!! May be inf!!
                weight_sum+=weight[k];
            }
    ...
    for(i=0; i<npts_x; i++)
    {
        for(j=0; j<npts_y; j++)
        {
            ...
            if(kkk<9*nsta*0.3||n[i][j]<nsta*0.3)
                continue;
            sprintf(t_name,"%sraw%g_%g.lst\0",arg[6],x0+i*dx,y0+j*dy);
