...
c forward FFT: fils ==> tmp
        ...
        do m = 1,ns
c  Modified by LF, original is tmp(m) = tmp(m)/ns
          tmp(m) = 2.0d0*tmp(m)/ns
        enddo
        j = 1
...
ccc Modified to output absolute amplitude, by LF
        arr1(9,i) = 10.0d0**( (ampgr(i)-100.0d0+amaxlf) /20.0d0 )
      enddo
      if(nfout2.ne.0) then
        if(nphpr.ne.0) then
     ...
     *                 phprvel,phgrc)
        ...
        do i = 1,nfout2
          ...
          arr2(7,i) = wdth1(i)
ccc Modified to output absolute amplitude, by LF
          arr2(8,i) = 10.0d0**( (ampgr1(i)-100.0d0+amaxlf) /20.0d0 )
        enddo
      else
        ierr = 2
      endif
